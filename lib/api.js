...
const getLiveHost = (options) => options.liveHost || config.url.liveHost || getHost(options);
...
const getURL = (action, params, options) => {
  ...
  const host = options.isLiveHost ? getLiveHost(options) : getHost(options);
...
const getEndURL = (options) => {
  ...
  return getURL('end', params, {...options, isLiveHost: true });
};
...
const getJoinURL = (username, options) => {
  ...
  return getURL('join', params, {...options, isLiveHost: false });
};
